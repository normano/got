...
import got, { RequestError, HTTPError } from '../source/index.js';
...
test('can retry without an agent', withServer, async (t, server, got) => {
    ...
    class MyAgent extends HttpAgent {
        createConnection(port, options, callback) {
            counter++;
            return HttpAgent.prototype.createConnection.call(this, port, options, callback);
        }
    }
    const { response } = (await t.throwsAsync(got({
        agent: {
            http: new MyAgent(),
        },
        hooks: {
