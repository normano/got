...
import got, { Options, RequestError } from '../source/index.js';
...
test('`url` should be utf-8 encoded', async (t) => {
    await t.throwsAsync(got('https://example.com/%D2%E0%EB%EB%E8%ED'), {
        instanceOf: RequestError,
...
test('merges `searchParams` instances', t => {
    ...
    // eslint-disable-next-line unicorn/prevent-abbreviations
    const searchParams = instance.defaults.options.searchParams;
...
test('options have url even if some are invalid', async (t) => {
    ...
    t.is((error?.options.url).href, 'https://example.com/');
...
test('options have url even if some are invalid - got.extend', async (t) => {
    const instance = got.extend({
        handlers: [
            (options, next) => {
                t.is(options.url.href, 'https://example.com/');
                return next(options);
            },
        ],
    });
