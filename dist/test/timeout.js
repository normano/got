...
import got, { TimeoutError } from '../source/index.js';
...
test('timeouts are emitted ASAP', async (t) => {
    ...
    const error = await t.throwsAsync(got('http://192.0.2.1/test', {
        retry: {
            limit: 0,
        },
        timeout: {
...
test('http2 timeout', async (t) => {
    const error = await t.throwsAsync(got('https://123.123.123.123', {
        timeout: {
            request: 1,
        },
        http2: true,
