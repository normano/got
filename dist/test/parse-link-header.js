...
test('works as expected', t => {
    t.deepEqual(parseLinkHeader('<https://one.example.com>; rel="preconnect", <https://two.example.com>; rel="preconnect", <https://three.example.com>; rel="preconnect"'), [
        {
    ...
    t.deepEqual(parseLinkHeader('<https://one.example.com>; rel="previous"; title="previous chapter"'), [
        {
            reference: 'https://one.example.com',
            parameters: { rel: '"previous"', title: '"previous chapter"' },
        },
    ]);
    t.deepEqual(parseLinkHeader('</>; rel="http://example.net/foo"'), [
        {
            reference: '/',
            parameters: { rel: '"http://example.net/foo"' },
        },
    ]);
    t.deepEqual(parseLinkHeader('</terms>; rel="copyright"; anchor="#foo"'), [
        {
            reference: '/terms',
