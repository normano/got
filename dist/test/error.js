...
test('catches dns errors', async (t) => {
    const error = (await t.throwsAsync(got('http://doesntexist', { retry: { limit: 0 } })));
    t.truthy(error);
...
test('no uncaught parse errors', async (t) => {
    ...
    await t.throwsAsync(got.head(`http://localhost:${server.address().port}`), {
        instanceOf: RequestError,
        message: /^Parse Error/,
    });
    await close();
...
test('no uncaught parse errors #2', async (t) => {
    ...
    await t.throwsAsync(got(`http://localhost:${server.address().port}`), {
        instanceOf: RequestError,
        message: /^Parse Error/,
    });
    await close();
